...
Page({
	...
	switchFontAddress(isBtn, cartVaildJson, callback) {
		...
		if (!isBtn) {
			UTIL.ajaxCommon(API.URL_ZB_CART_GOODSVALID, cartVaildJson, {
				complete: (res) => {
    ...
    renderCartPage() {
        ...
        function cartRender() {
            ...
            if (!wx.getStorageSync('yunchaoCartList') || JSON.parse(wx.getStorageSync('yunchaoCartList')).length == 0) {
                ...
                // 校验自提点对应门店数据
				that.switchFontAddress(0, cartInputJson, (cartVaildJson) => {
					// 重置购物车入参数据
					...
					UTIL.ajaxCommon(API.URL_ZB_CART_GOODSVALID, cartInputJson, {
						"complete": (res) => {
							...
							if (res && res._code == '003017') {
								...
								wx.setStorageSync('yunchaoCartList', that.data.overStockLmit)
								that.renderCartPage();
								return;
							}
							cartOutputJson = res;
                ...
                // // 调用接口
                // UTIL.ajaxCommon(API.URL_ZB_CART_GOODSVALIDCOMMUNITY, cartInputJson, {
                //     "complete": (res) => {
                //         cartOutputJson = res;
                //         cartjson();
                //     }
    ...
    toAddress() {
        ...
        if (!UTIL.isLogin()) {
            ...
            wx.navigateTo({
                url: "/pages/user/address/list/list?from=cart&b2cType=1&isCrossBorder=2"
            });
        } else {
            wx.navigateTo({
                url: "/pages/user/address/list/list?from=cart&b2cType=1&isCrossBorder=2"
        ...
        if (that.data.clickGoFill) {
            if (!UTIL.isLogin()) {
                ...
                if (!that.data.addressId && that.data.addressId !== "0") {
                    ...
                    if (foodNum > 0 && that.data.foodDelivery == 1 || marketNum > 0 && that.data.goodsDelivery == 1) {
                ...
                UTIL.ajaxCommon(API.URL_ZB_CART_GOODSVALIDCOMMUNITY, inputJson, {
                    "complete": (res) => {
                        getPayJson = res;
                        ...
                        function getPayJsonTest() {
                            if (getPayJson._data && getPayJson._code == API.SUCCESS_CODE) {
                                ...
                                if (!canHref) {
                                    ...
                                    // if (type) {
                                    //     wx.navigateTo({
                                    //         url: '/pages/user/wxLogin/wxLogin?reffer=' + encodeURIComponent('/pages/yunchao/fill/fill?b2cType=1&goodsDelivery=' + goodsDelivery + '&foodDelivery=' + foodDelivery + '&goodsB2CDelivery=' + goodsB2CDelivery + '&currentDelivery=' + currentDelivery + '&from=cart')
                                    //     });
                                    // } else {
                                    //     wx.navigateTo({
                                    //         url: '/pages/user/wxLogin/wxLogin?reffer=' + encodeURIComponent('/pages/order/fill/fill?goodsDelivery=' + goodsDelivery + '&foodDelivery=' + foodDelivery + '&goodsB2CDelivery=' + goodsB2CDelivery + '&currentDelivery=' + currentDelivery + '&from=cart')
                                    //     });
                                    // }
                                } else {
                                    let type = '';
                                    ...
                                    if(getPayJson._data.b2cHsNum > 0){
                                        that.selectSupplierToPay(JSON.parse(wx.getStorageSync('yunchaoFillCartList') || '[]'))
                                    } else {
                                        that.setData({
                                            clickGoFill: false
                                        });
                                        wx.navigateTo({
                                            url: `/pages/yunchao/fill/fill?b2cType=1&foodDelivery=1&goodsDelivery=1&goodsB2CDelivery=1&currentDelivery=-1&from=cart`
                                        });
                                    }
                                    // if (type) {
                                    //     wx.navigateTo({
                                    //         url: `/pages/yunchao/fill/fill?b2cType=1&foodDelivery=${foodDelivery}&goodsDelivery=${goodsDelivery}&goodsB2CDelivery=${goodsB2CDelivery}&currentDelivery=${currentDelivery}&from=cart`
                                    //     });
                                    // } else {
                                    //     wx.navigateTo({
                                    //         url: `/pages/order/fill/fill?foodDelivery=${foodDelivery}&goodsDelivery=${goodsDelivery}&goodsB2CDelivery=${goodsB2CDelivery}&currentDelivery=${currentDelivery}&from=cart`
                                    //     });
                                    // }
                                }
                            } else {
     ...
     * 选择一家供应商
     */
    selectSupplierToPay(supplierGoodsList, ValStoreList, callback){
        ...
        if(supplierGoodsList.length > 1){
            ...
            console.log(supplierGoodsList[0])
    ...
    selectSupplierItem(e,paramId){
        ...
        let storeList = JSON.parse(wx.getStorageSync('yunchaoFillCartList') || '[]');
    ...
    switchDeliveryGlobal(event) {
        ...
        let newJson = that.data.newCartJson;
        if(goodsDelivery==0){
            newJson.fillAddress=that.data.ztAddress[0]
        }
        // wx.setStorageSync('fillAddress', JSON.stringify(that.data.ztAddress[0]));
