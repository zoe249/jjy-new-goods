...
Page({
	data: {
		...
		// 判断全部商品是否都加入了购物车
		allShop: true,
		//埋点数据页面ID --  A1007优鲜食谱详情  A2007 社团食谱详情
		currentPageId: '',
		// 页面的分类id
		sectionId: "",
	...
	onLoad(e) {
		...
		if (e.contentId) {
			this.setData({
				...
				entrance: e.entrance,
				//埋点数据页面ID
				currentPageId: e.entrance == 0 ? 'A1007' : 'A2007'
			})
		} else {
			this.setData({
	...
	// 分享设置
	onShareAppMessage(e) {
		let url = '/pages/AA-RefactorProject/pagesSubcontract/RecipeDetails/index?contentId=' + this.data
		...
		let operationId = entrance == 0 ? 'D1027' : 'D1032';
		UTIL.jjyFRLog({
	...
	GetDetail() {
		let APIContent = null
		if (this.data.entrance == 0) {
			APIContent = API.GET_COOK_BOOKV2
		} else {
			APIContent = API.GET_COOK_BOOKV2_ZB
		}
		UTIL.ajaxCommon(APIContent, {
			...
			success: (res) => {
				if (res._code == API.SUCCESS_CODE) {
					...
					// 对食材清单数据进行处理
					if (obj.material) {
						if (!obj.material.auxiliary) {
							obj.material.auxiliary = []
						}
						if (!obj.material.master) {
							obj.material.master = []
						}
						if (obj.material.auxiliary.length > obj.material.master.length) {
							let num = obj.material.auxiliary.length - obj.material.master.length
							for (let i = 0; i < num; i++) {
								obj.material.master.push({
									id: '-1',
									name: '占位',
									amount: '占位'
								})
							...
							let num = obj.material.master.length - obj.material.auxiliary.length
							for (let i = 0; i < num; i++) {
								obj.material.auxiliary.push({
									id: '-1',
									name: '占位',
									amount: '占位'
								})
									...
									if (item.cookbookMaterialJson.master.length != 0 ||
										...
										item.cookbookMaterialJson.master.forEach((items,
											...
											item.main_ingredient = item
												.main_ingredient + '、' + items.name
										})
										item.main_ingredient = item.main_ingredient.slice(1)
									}
								} else {
									item.main_ingredient = ''
								}
		...
		UTIL.ajaxCommon(APIContent, {
			...
			success: (res) => {
				if (res._code == API.SUCCESS_CODE) {
					...
					let nav_list = []
					// 判断食材清单
					if (this.data.listDetail.material) {
						nav_list.push({
							id: 1,
							name: '食材清单',
							className: '.Ingredients_list'
						})
					}
					// 判断商品清单
					...
					// 判断烹饪小技巧
					if (this.data.listDetail.cookbookCookingSkill) {
						nav_list.push({
							id: 4,
							name: '烹饪小技巧',
							className: '.cookingTips'
						})
					}
	...
	ScrollFunY() {
		//埋点数据-相关商品
		let entrance = this.data.entrance; //0 优鲜 1 社区团购
		let operationId = entrance == 0 ? 'D1043' : 'D1045';
		UTIL.jjyFRLog({
	...
	btnCollect() {
		if (this.data.isclick) {
			...
			//埋点数据--收藏
			let entrance = this.data.entrance; //0 优鲜 1 社区团购
			let operationId = entrance == 0 ? 'D1026' : 'D1031';
			UTIL.jjyFRLog({
				clickType: 'C1003', //点击事件
				conType: 'B1004', //动作类型：按钮维度
				operationId: operationId,
	...
	addCart(event) {
		...
		if (type && type == 'ingredients') {
			operationId = entrance == 0 ? 'D1025' : 'D1030';
		} else { //商品清单
			operationId = entrance == 0 ? 'D1024' : 'D1029';
		}
		UTIL.jjyFRLog({
			clickType: 'C1003', //点击事件
			conType: 'B1004', //动作类型：按钮维度
	...
	OneAddCart(event) {
		...
		//埋点数据-一键加购
		let entrance = this.data.entrance; //0 优鲜 1 社区团购
		let operationId = entrance == 0 ? 'D1023' : 'D1028';
		UTIL.jjyFRLog({
			clickType: 'C1003', //点击事件
			conType: 'B1004', //动作类型：按钮维度
			operationId: operationId,
