...
Page({
  ...
  reloadPage: function() {
    ...
    UTIL.ajaxCommon(API.URL_ZB_BROKERAGE_ANTICIPATEREVENUELIST, data, {
      "success": (res) => {
        if (res && res._code == API.SUCCESS_CODE) {
          if (res._data.length > 0) {
            for (let i = 0; i < res._data.length; i++) {
              ...
              res._data[i].brokerageStatusStr = $brokerageStatusStr(res._data[i].brokerageStatus);

              //res._data[i].statusName = $cashStatusNameStr(res._data[i].orderStatus);
            }
          }
          if (page == 1) {
            list = res._data;
