...
let groupNowOrderType = wx.getStorageSync('groupNowOrderType') || 0;
let APP = getApp();
...
Page({
  ...
  confirmSearch: function(event) {
    ...
    groupNowOrderType = event.target.dataset.orderState;
    wx.setStorageSync('groupNowOrderType', groupNowOrderType);
    this.setData({
  ...
  toRefundDetail: function(event) {
    ...
    wx.setStorageSync('groupNowOrderType', that.data.orderStatus);
  ...
  /*售后记录*/
  toRecord: function() {
    wx.setStorageSync('groupNowOrderType', 0);
    this.setData({
      showNav: false
    });
    wx.setStorageSync("selfOrderType", 1);//自提点的类型：2，合伙人：1
  ...
  reloadPage(options) {
    ...
    wx.setStorageSync('groupNowOrderType', that.data.orderStatus);
    orderListData.orderStatus = groupNowOrderType || that.data.orderStatus;
    orderListData.searchStr = that.data.confirmNamePhone;
    ...
    orderListData.searchStr = that.data.confirmNamePhone;
    // orderListData.shopId = APP.globalData.groupShopInfo.shopId;
    // orderListData.warehouseId = APP.globalData.groupShopInfo.warehouseId;
    // orderListData.centerShopId = APP.globalData.groupShopInfo.centerShopId;
    // orderListData.centerWarehouseId = APP.globalData.groupShopInfo.centerWarehouseId;
    UTIL.ajaxCommon(API.URL_ZB_ORDER_GROUPLIST, orderListData, {
      "success": (res) => {
  ...
  onLoad: function(options) {
    ...
    groupNowOrderType = newOptions.urlOrderType || wx.getStorageSync('groupNowOrderType') || 0
    let orderStatus = newOptions.urlOrderType || 0;
    wx.setStorageSync('groupNowOrderType', groupNowOrderType);
    that.setData({
      page: 1,
      orderStatus: groupNowOrderType,
  ...
  onShow() {
    ...
    groupNowOrderType = wx.getStorageSync('groupNowOrderType') || 0;
    orderListData.orderStatus = groupNowOrderType || 0;
    orderListData.page = 1;
    that.setData({
      page: 1,
