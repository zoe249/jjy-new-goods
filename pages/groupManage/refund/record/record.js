...
Page({
  ...
  renderPage: function () {
    ...
    let customerserviceListData = {
      ...
      page: page,
      rows: rows,
      type: wx.getStorageSync('selfOrderType') || 1,
    };
          ...
          if (res && res._code == API.SUCCESS_CODE) {
            ...
            if (res._data.length > 0 && res._data.length == rows) {
              ...
              that.setData({
                ...
                canPullDown: false,
                hasNextPage: false,
                errorPageMsg: page == 1 && res._data.length == 0 ? '暂无售后订单' : '',
              });
            }
          } else {
            that.setData({
  ...
  selfPointReceipt(e) {
    ...
    UTIL.ajaxCommon(API.URL_CUSTOMERSERVICE_SELFPOINTRECEIPT, {
      customerServiceId: item.customerServiceId
    }, {
      success: (res) => {
        if (res._code == API.SUCCESS_CODE) {
          listArr[index].selfPointReceiptFlag = false
          this.setData({
            listArr
          })
