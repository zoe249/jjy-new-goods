...
Page({
    ...
    UTIL.ajaxCommon(API.URL_ZB_GROUPADDRESS_GROUPMEMBERSHOPLIST, data, {
      success: (res) => {
        if (res._code === API.SUCCESS_CODE && !!res._data) {
          let allowShopLimit = true
          if (res._data.length <= 0) {
            allowShopLimit = false
          ...
          that.setData({
            ...
            shopId: currentShop && currentShop.shopId ? currentShop.shopId : '',
            warehouseId: currentShop && currentShop.warehouseId ? currentShop.warehouseId : 0
          });
          if (!currentShop) return;
          UTIL.queryShopByShopId(currentShop, function (shopObj) {
      ...
      complete: function (res) {
        if (res._msg && res._code !== API.SUCCESS_CODE) {
          APP.showToast(res._msg)
        }
      }
    })
  ...
  bindSelectorChange(e) {
    ...
    UTIL.queryShopByShopId(groupShopInfo, function (shopObj) {
        wx.setStorageSync('shop_attribute', shopObj.shopAttribute)
      that.iniLoadData().then(() => {
        //邀请团长scene
        that.getSceneAddShareInfo();
        //初始化数据
        that.renderCurrentPage();
        ...
        'success': (res) => {
          if (res._code == API.SUCCESS_CODE) {
            if (res._data && res._data.length) {
              group.cur.page++;
              let lt = [];
              res._data.map(function (items, index) {
      ...
      UTIL.ajaxCommon(API.URL_ZB_PROMOTIONCOLONEL_QUERYPANICBUYINGPROMOTIONFORGOODS, {
        ...
        'success': (res) => {
          if (res._code == API.SUCCESS_CODE) {
            ...
            if (seckill.cur.list.length == 0 && list.length == 0) {
              empty = true
            }
            let listKey = `seckill.soon.list`;
            let pageKey = `seckill.soon.page`;
            let noMoreKey = `seckill.soon.noMore`
            let emptyKey = `seckill.empty`
            that.setData({
              [listKey]: list,
              [pageKey]: page,
              [noMoreKey]: noMore,
        ...
        complete: function (res) {
          if (res._code && res._code !== API.SUCCESS_CODE) {
            APP.showToast(res._msg);
            if (cur.list.length == 0 && list.length == 0) {
              let emptyKey = `seckill.empty`
              that.setData({
                [emptyKey]: true
              })
 ...
 /**
  ...
  drawShareHomeCard(){
    ...
    let Draws = new DrawShareCard({qrCode: sharePoster.xcxCodeUrl})
  ...
  drawShareCard(e) {
    ...
    let Draws = new DrawShareCard({list:drawData.list, qrCode: drawData.qrCode})
  ...
  moreExtension(e) {
    ...
    if ((swiperNavActive == 0 || swiperNavActive == 1) && more == 2) {
      switch (swiperNavActive) {
        case 0:
          wx.navigateTo({
            url: `/pages/groupManage/groupList/groupList?shopId=${UTIL.getShopId()}&shareMemberId=${UTIL.getMemberId()}&isExtend=2`,
          })
          break;
        case 1:
          wx.navigateTo({
            url: `/pages/groupManage/panicBuying/panicBuying?shopId=${UTIL.getShopId()}&shareMemberId=${UTIL.getMemberId()}&isExtend=2`,
          })
          break;
      }
      return
    ...
    if (more == 1 || more == 4) {
      ...
      if (swiperNavActive == 2 && discount.cur.list.length >= 1) {
        ...
        that.setData({
          [`drawData.list`]: discount.cur.list
        })
      }
          ...
          that.downloadNeedFiles(downLoadObj, function (dowLoadImg) {
            that.initShareImage(dowLoadImg, downLoadObj, function (downLoadShareImg) {
              ...
              if ((swiperNavActive == 2 || swiperNavActive == 3) && more == 2 && param.skuIdList.length > 1) {
                wx.navigateTo({
                  url: `/${shareShortLinkGbOutput.path}&isExtend=2&code=${shareShortLinkGbOutput.xcxCodeUrl}`,
                })
                return
              ...
              let shareDeatails = {
                path: shareShortLinkGbOutput.path+'&fenxiang=1',
  ...
  countDownFun() {
    ...
    let proEndTimeArr = []
    if (dataArr.cur.list.length > 0) {
      dataArr.cur.list.map(function (items) {
        items.proEndTimeNum = items.proEndTime + 'd'
        if (proEndTimeArr.indexOf(items.proEndTime) == -1) {
          proEndTimeArr.push(items.proEndTime)
        }
      })
    }
    // if (dataArr.soon.list.length > 0) {
    //   dataArr.soon.list.map(function (items) {
    //     items.proEndTimeNum = items.proBeginTime + 'd'
    //     if (proEndTimeArr.indexOf(items.proBeginTime) == -1) {
    //       proEndTimeArr.push(items.proBeginTime)
    //     }
    //   })
    // }
    that.initSurplusTimes(proEndTimeArr)
    that.setData({
      [dataAreName]: dataArr
    })
  ...
  //进行中倒计时
  initSurplusTimes(timeArr) {
    let that = this;
    var curtime = Date.parse(new Date());
    var timerList = {}
    timeArr.map(function (time) {
      let second = Math.floor((time - curtime) / 1000);
      timerList[`${time}d`] = that.formatData(second)
      // timerList[`${time}d`]={'str':"00:00:00",'d_str':0}
    })
    that.setData({
      timerList: timerList
    })
    that.data.timerFunId = setInterval(() => {
      curtime += 1000
      timeArr.map(function (time) {
        let second = Math.floor((time - curtime) / 1000);
        timerList[`${time}d`] = that.formatData(second)
  ...
  // 清除定时器
  clearAllTimer() {
    clearInterval(this.data.timerFunId)
    this.setData({
      timerList: []
    })
  },
  ...
  formatData(second) {
    ...
    second %= 60;
    var str = this.toDouble(hour) + ':' +
      this.toDouble(minute) + ':' +
      this.toDouble(second);
    var d_str = this.toDouble(day);
    return {
      str,
      d_str
    }
    ...
    if (!canIUse) {
      wx.setStorage({
        ...
        success: () => {
          that.onLeave();
          wx.navigateTo({
            url: `/pages/groupManage/extendSearch/extendSearch?navTitle=${swiperNavItem[swiperNavActive].title}&searchGoodsName=${searchGoodsName}&swiperNavActive=${swiperNavActive}&shopId=${UTIL.getShopId()}`,
          })
        }
      })
      return;
    ...
    UTIL.ajaxCommon(API.URL_ZB_PCQG_FORNAME, data, {
      success: (res) => {
        if (res._data) {
          ...
          that.clearAllTimer()
          let proEndTimeArr = []
          search.list.map(function (items) {
            items.proEndTimeNum = items.proEndTime + 'd'
            if (proEndTimeArr.indexOf(items.proEndTime) == -1) {
              proEndTimeArr.push(items.proEndTime)
            }
          })
          that.countDownFun(proEndTimeArr)
          this.setData({
            search,
            searchShow: true
          })
