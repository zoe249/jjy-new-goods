...
Page({
  ...
  onLoad: function(options) {
    const {
      categoryId,
      categoryName ="%E5%88%B0%E5%BA%97%E5%90%83",
      secondCateId,
      shopId='',
      selectSectionId=''
    } = options;
  ...
  closeRightNav: function (event) {
    this.setData({
      showRightNav: false,
    });
  }, 
  clickRightNav: function (event) {
    let { rightNavId}=event.currentTarget.dataset;
    let { selectRightNavId}=this.data
    this.setData({
      selectRightNavId: rightNavId,
      showRightNav:false,
    });
  }, 
  // 
  rightNavToggle: function (event) {
    let { showRightNav } = this.data;
   this.setData({
  ...
  initList: function() {
    ...
    UTIL.ajaxCommon(API.URL_GOODS_FOODDINEIN, {
      firstCateId,
      page: 1,
      rows,
      delivery: "",
  ...
  changeTopNav: function(event) {
    ...
    if (field !== sortList[0].field) {
      sortList[0].field = field;
      sortList[0].sort = sort;
      list = [];
      page = 1;
      haveNextPage = true;
      this.setData({
