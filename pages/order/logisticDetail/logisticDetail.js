...
Page({
  ...
  onLoad: function (options) {
    ...
    let oData={orderStoreId:options.orderStoreId,orderId:options.orderId}
    UTIL.ajaxCommon(API.URL_ORDER_DETAIL, oData, {
      "complete": (result) => {
        console.log(result)
        this.setData({
          addrFull:result._data.addrFull,
          addrMobile:result._data.addrMobile,
          addrName:result._data.addrName
        })
      }
    })
  },
  ...
  onShow: function () {
    ...
    if(options.isB2C==1634){
        let {orderId,orderStoreId}=options
        var data={orderId,orderStoreId}
        UTIL.ajaxCommon(API.URL_ZB_QUERYYCLOGISTICSLOG,data,{
          success(res){
            for(let i in res._data){
              res._data[i].statusMsg=API.LOGISTICS_STATUS[res._data[i].status]
            }
            that.setData({
              logisticsList: res._data,
            });
          },
          complete() {
            that.setData({
