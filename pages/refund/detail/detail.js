...
Page({
  ...
  cancelRefundConfirm: function () {
      ...
      let {result,refundInfoId,canClick}=that.data;
      let cancelData={
  ...
  confirmSubmit:function () {
    let that=this;
    let {canClick,result,logisticsCompanyName,logisticsOddNumber,customerReturnFreight,logisticsCompanyValue}=that.data;
    if(canClick){
        let saveSubmitData={
...
renderPage:function(){
  ...
  UTIL.ajaxCommon(API.URL_CUSTOMERSERVICE_DETAILS,{
    ...
    success(res){
      if (res&&res._code == API.SUCCESS_CODE) {
        ...
        result.groupMode = result.groupMode||'';
  ...
  switchNav:function(event){
    let {tab}=event.currentTarget.dataset;
    this.setData({
      tab:tab,
    });
  },
  ...
  previewImage:function(event){
    let that=this;
    noOnShow=true;
    let {urlArr,url}=event.currentTarget.dataset;
    wx.previewImage({
