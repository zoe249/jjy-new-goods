...
*/
  ...
  btnSubmit(){
    ...
    if(that.data.refundReason==-1){
      ...
      if (canSubmit){
        ...
        canSubmit=false;
        let url=API.URL_CUSTOMERSERVICE_SAVE;
        if(shippingTypeNow==1022||shippingTypeNow==1023||shippingTypeNow==1024){
          url=API.URL_OVERSEASCUSTOMERSERVICE_SAVE
        }
        UTIL.ajaxCommon(url, saveData, {
          success(res) {
            if (res && res._code == API.SUCCESS_CODE) {
              wx.requestSubscribeMessage({
                tmplIds: [API.MSG_SUBSCRIBEMESSAGE_REFUND],
                success (res) { },
                fail (resMessage) {
                  APP.showToast(resMessage&&resMessage.errMsg?resMessage.errMsg:'订阅售后消息失败');
                  },
                complete(){
                  ...
                  wx.redirectTo({
                    url: `/pages/refund/success/success?from=refundSubmit&servicePhone=${servicePhone}&shippingTypeNow=${shippingTypeNow}&id=${id}&aftermarketType=${aftermarketType}&createTime=${createTime}&goodsSence=${goodsSence}`
                  });
                }
              });
            } else {
  ...
  selectRefundReason(event){
    ...
    let { shippingTypeNow, detailData, refundSkuJson, canShowToHome, isNeedPickup, refundNum } = this.data;
    canShowToHome = true;
    ...
    UTIL.ajaxCommon(API.URL_ORDER_DETAIL, oData,{
      success(res){
        if (res&&res._code == API.SUCCESS_CODE) {
          ...
          userName = res._data.memberNikeName || wx.getStorageSync('nickName') || res._data.addrName  || '';
          userPhone = res._data.memberMobile || wx.getStorageSync('mobile') || res._data.addrMobile || '';
  ...
  onShowInit:function(){
    let that=this;
    let refundSkuJson=that.data.refundSkuJson;//{"nowSkuList":{"goodsId":10328,"goodsSkuId":10328,"goodsName":"【927】王老吉凉茶
